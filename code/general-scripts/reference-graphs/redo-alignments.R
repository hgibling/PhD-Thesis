...
setwd("/.mounts/labs/simpsonlab/users/hgibling/graph-alignments-new-stats/jq/stats")

graph.dir <- "/.mounts/labs/simpsonlab/users/hgibling/graph-alignments-new-stats/jq/"

graph.names <- c("allele-stacked", "hg17")
...
get.scores <- function(allele) {
    ...
    data <- read.csv(file, header=F, stringsAsFactors=F,
        ...
        mutate(Score=ifelse(Score=="null", 0, Score)) %>%
