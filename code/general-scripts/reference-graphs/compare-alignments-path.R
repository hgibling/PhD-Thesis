...
graph.names <- c("allele-msa-path", "allele-msa-dag-path")
allele.names <- c(LETTERS[1:5], paste0("L", 1:24), paste0("L", 32:38)) 

get.scores <- function(allele) {
    file <- paste0(graph.dir, allele, "-graph-path-alignments.csv")
...
save.name <- paste0(args[1], "-path.csv")
