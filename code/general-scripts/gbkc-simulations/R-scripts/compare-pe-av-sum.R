...
setwd("~/awadalla/PRDM9-Project/HMM/sim-reads/pe-lam")

allele.order <- c(LETTERS[1:5], paste0("L", 1:24), paste0("L", 32:38))

av.combined <- read.csv("av-pe-pr.csv", header=F, stringsAsFactors=F,
...
sum.combined <- read.csv("sum-pe-pr.csv", header=F, stringsAsFactors=F,
                        col.names=c("Precision", "Recall", "Coverage", 
                                    "Error", "kmer", "Iteration")) %>%
    group_by(Coverage, Error, kmer) %>%
    summarize(MeanPrecision=mean(Precision), MeanRecall=mean(Recall)) %>%
...
combined <- bind_rows(av.combined, sum.combined, counts.combined)


av.each <- read.csv("av-pe-pr-each.csv", header=F, stringsAsFactors=F,
...
sum.each <- read.csv("sum-pe-pr-each.csv", header=F, stringsAsFactors=F,
                    col.names=c("Allele", "Precision", "Recall", "Accuracy", "Coverage", 
                                "Error", "kmer", "Iteration")) %>%
    group_by(Allele, Coverage, Error, kmer) %>%
    summarize(MeanPrecision=mean(Precision), MeanRecall=mean(Recall)) %>%
...
each <- bind_rows(av.each, sum.each, counts.each)

combined$Error <- factor(combined$Error, levels=c(0.005, 0.002, 0.001, 0))
combined$Scoring <- factor(combined$Scoring, levels=c("Average", "Sum", "Count"))
each$Error <- factor(each$Error, levels=c(0.005, 0.002, 0.001, 0))
each$Scoring <- factor(each$Scoring, levels=c("Average", "Sum", "Count"))
