...
countdir=$prdm9dir/simulations/counts-diploid
distdir=$prdm9dir/simulations/distances-diploid
simdir=$prdm9dir/simulations/combine-counts-distances-diploid
...
for i in $(seq 1 $5)
do
    if [ ! -s $simdir/$2/scores/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-$8-scores-combined.csv ]
    then
        $codedir/combine-haploid-counts-distances-scores.py -d $distdir/$2/scores/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-$8-scores.csv -c $countdir/$2/scores/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-scores.csv -o $simdir/$2/scores/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-$8-scores-combined
        if [ ! -s $simdir/$2/scores/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-$8-scores-combined.csv ]; then fail "Counts and distances scores could not be combined for iteration $i"; fi
    fi
    ...
    then
        $codedir/precision-recall-k-range.py -s $simdir/$2/scores/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-$8-scores-combined.csv -o $simdir/$2/pr/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-$8-scores-pr
        ...
        if [ ! -s $simdir/$2/pr/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-$8-scores-pr-labeled.csv ]; then fail "Precision and recall could not be completed for iteration $i"; fi
	rm $simdir/$2/pr/all-genotypes-flank10k-$3-bp-$6-frag-$2-x-e-$4-i-$i-F-$7-k-$8-scores-pr.csv
    fi
    echo "Precision and recall for iteration $i complete"
done


# combine meta scores and determine how many times genotypes called correctly across iterations
