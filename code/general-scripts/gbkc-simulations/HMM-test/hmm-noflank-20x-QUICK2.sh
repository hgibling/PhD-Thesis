...
i=$(awk "NR==$SGE_TASK_ID" $hmmdir/quick-list2.tsv | cut -f2)
a=$(awk "NR==$SGE_TASK_ID" $hmmdir/quick-list2.tsv | cut -f1)

python $hmmdir/call-alleles.py -a $hmmdir/all-alleles.csv -r $readdir/$a-noflank-100-bp-250-frag-20-x-e-0-i-$i.r12.reads -m 0.0000000001 -o $savedir/$a-noflank-100-bp-250-frag-20-x-e-0-i-$i-scores
