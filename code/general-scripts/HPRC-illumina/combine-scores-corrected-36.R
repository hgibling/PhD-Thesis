...
count.path <- paste(illumina.path, "gbkc-calls-corrected-36", sep="/")
distance.path <- paste(illumina.path, "gbkc-distance-calls-corrected-36", sep="/")

# Files
count.data <- read.table(paste(count.path, 
                               paste0(sample, ".filtered-corrected-all-methods.tsv"), sep="/"), 
                               col.names=c("CountMethod", "k", 
                               "Genotype", "CountScore"))
distance.data <- read.table(paste(distance.path, 
                            paste0(sample, ".filtered-corrected-paired-all-methods.tsv"), sep="/"),
...
write.table(combined, paste0(illumina.path, "/combined-calls-corrected-36/", sample,
