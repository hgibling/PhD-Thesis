...
time $gbkcdir/gbkc distance -a $prdm9dir/all-alleles-flank10k.fa -1 $fqdir/$sample.illumina-prdm9-10k-filtered-paired-corrected.r1.fa -2 $fqdir/$sample.illumina-prdm9-10k-filtered-paired-corrected.r2.fa -d -K $k -e $err -c $cov -f $frag -s $std -l $len -m max -o $savedir/$sample.filtered-paired-max-PRDM910k-stats-flank.tsv
