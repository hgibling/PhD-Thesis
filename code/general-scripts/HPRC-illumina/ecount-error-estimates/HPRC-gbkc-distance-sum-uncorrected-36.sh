...
time $gbkcdir/gbkc distance -a $prdm9dir/all-alleles-flank10k.fa -1 $fqdir/$sample.illumina-prdm9-10k-filtered-paired.r1.fq -2 $fqdir/$sample.illumina-prdm9-10k-filtered-paired.r2.fq -d -K $k -f $prdm9dir/flank10k.fa -m sum -e $err -c $cov -f $frag -s $sdev -l $len -o $calldir/$sample.filtered-paired-sum-ecount-uncorrected-36.tsv
