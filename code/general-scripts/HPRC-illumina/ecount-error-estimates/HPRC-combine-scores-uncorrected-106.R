...
base.path <- "/.mounts/labs/simpsonlab/users/hgibling/PAPER/HPRC/illumina/rerun-gbkc"
count.path <- paste(base.path, "count-calls-uncorrected-106", sep="/")
distance.path <- paste(base.path, "distance-calls-uncorrected-106", sep="/")

# Files
count.data <- read.table(paste(count.path, 
                               paste0(sample, ".filtered-paired-ecount-count-uncorrected-106.tsv"), sep="/"), 
                               col.names=c("CountMethod", "k", "Genotype", "CountScore"), sep="\t")
distance.data <- read.table(paste(distance.path, 
                            paste0(sample, ".filtered-paired-ecount-distance-uncorrected-106.tsv"), sep="/"),
...
write.table(combined, paste0(base.path, "/combined-calls-uncorrected-106/", sample,
                             ".filtered-paired-ecount-combined-uncorrected-106.top100.tsv"),
