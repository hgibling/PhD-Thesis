...
count.path <- paste(base.path, "count-calls-corrected-36", sep="/")
distance.path <- paste(base.path, "distance-calls-corrected-36", sep="/")

# Files
count.data <- read.table(paste(count.path, 
                               paste0(sample, ".filtered-paired-ecount-count-corrected-36.tsv"), sep="/"), 
                               col.names=c("CountMethod", "k", "Genotype", "CountScore"), sep="\t")
distance.data <- read.table(paste(distance.path, 
                            paste0(sample, ".filtered-paired-ecount-distance-corrected-36.tsv"), sep="/"),
...
write.table(combined, paste0(base.path, "/combined-calls-corrected-36/", sample,
                             ".filtered-paired-ecount-combined-corrected-36.top100.tsv"),
