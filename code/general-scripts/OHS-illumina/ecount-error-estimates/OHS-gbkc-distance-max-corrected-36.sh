...
time $gbkcdir/gbkc distance -a $prdm9dir/all-alleles.fa -1 $fqdir/$sample.filtered-illumina-prdm9-10k-paired-corrected.r1.fa -2 $fqdir/$sample.filtered-illumina-prdm9-10k-paired-corrected.r2.fa -d -K $k -f $prdm9dir/flank10k.fa -m max -e $err -c $cov -f $frag -s $sdev -l $len -o $calldir/$sample.filtered-paired-max-ecount-corrected-36.tsv
