...
time $gbkcdir/gbkc count -a $prdm9dir/all-alleles-standard.fa -1 $fqdir/$sample.filtered-illumina-prdm9-10k-paired-corrected.r1.fa -2 $fqdir/$sample.filtered-illumina-prdm9-10k-paired-corrected.r2.fa -d -K $k -f $prdm9dir/flank10k.fa -m coverage -e $err -c $cov -l $len -o $savedir/$sample.filtered-corrected-coverage-PRDM910k-stats-flank-corrected.tsv
