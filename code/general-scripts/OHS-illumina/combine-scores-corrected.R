...
count.path <- paste(base.path, "gbkc-count-calls-corrected", sep="/")
distance.path <- paste(base.path, "gbkc-distance-calls-corrected", sep="/")

# Files
count.data <- read.table(paste(count.path, 
                               paste0(sample, ".filtered-paired-all-methods-corrected.tsv"), sep="/"), 
...
write.table(combined, paste0(base.path, "/combined-calls-corrected/", sample,
