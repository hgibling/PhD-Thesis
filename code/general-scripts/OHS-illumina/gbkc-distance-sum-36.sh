...
time $gbkcdir/gbkc distance -a $prdm9dir/all-alleles-flank10k.fa -1 $fqdir/$sample.illumina-prdm9-10k-filtered-paired.r1.fq -2 $fqdir/$sample.illumina-prdm9-10k-filtered-paired.r2.fq -d -K $k -e $err -c $cov -f $frag -s $std -l $len -m sum -o $savedir/$sample.filtered-paired-sum-PRDM910k-stats-flank.tsv
