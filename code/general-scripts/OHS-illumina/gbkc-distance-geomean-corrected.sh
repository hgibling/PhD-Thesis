...
savedir=$basedir/gbkc-distance-calls-corrected
...
time $gbkcdir/gbkc distance -a $prdm9dir/all-alleles-standard-flank10k.fa -1 $fqdir/$sample.filtered-illumina-prdm9-10k-paired-corrected.r1.fa -2 $fqdir/$sample.filtered-illumina-prdm9-10k-paired-corrected.r2.fa -d -K $k -e $err -c $cov -f $frag -s $std -l $len -m geomean -o $savedir/$sample.filtered-paired-geomean-PRDM910k-stats-flank-corrected.tsv
